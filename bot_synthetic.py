...
def analyze_fibonacci(candles):
    ...
    if fib_786 <= price <= fib_618 and price > avg_open:
        return "BUY", [f"Fibo: zone achat [{fib_786:.4f}-{fib_618:.4f}]"]
    fib_618s = swing_low + 0.618 * rang
    fib_786s = swing_low + 0.786 * rang
    if fib_618s <= price <= fib_786s and price < avg_open:
        return "SELL", [f"Fibo: zone vente [{fib_618s:.4f}-{fib_786s:.4f}]"]
    return None, []

def get_h1_bias(candles_h1):
    if len(candles_h1) < 6:
        return None
    recent = candles_h1[-6:]
...
def compute_sl_tp(direction, candles_m1, entry):
...
async def analyze_instrument(key, info):
    symbol = info["symbol"]
...
async def run_bot():
    ...
    logger.info("Bot Synthetiques demarre")
    await bot.send_message(
        chat_id=CHAT_ID,
        text=(
            "🤖 *Bot Indices Synthetiques Deriv*\n"
            "━━━━━━━━━━━━━━━━\n"
            "📊 V10/V25/V50/V75/V100\n"
            "💥 Boom 300/500/600/900/1000\n"
            "💥 Crash 300/500/600/900/1000\n"
            "🦘 Jump 10/25/50/75/100 · Step\n"
    ...
    last_signal = {}
    while True:
        logger.info("Analyse synthetiques...")
        ...
        for key, result in zip(INSTRUMENTS.keys(), results):
            ...
            try:
                ...
                logger.info(f"Signal: {key} {sig.direction} confluence={sig.confluence}")
            except Exception as e:
